...
public class PremiereDao {
    ...
    public List<Premiere> getPremiere(String fromDate, String toDate) {
        List<Premiere> premieres = new ArrayList<>();
        StringBuilder sqlBuilder = new StringBuilder("SELECT * FROM premiere WHERE CAST(date as date)");
        ...
        if (toDate != "") {
            if (isHaveFromDate) {
                sqlBuilder.append(" AND CAST(date as date) <= '" + toDate + "'");
            } else {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                premiere.setId(Integer.parseInt(rs.getString(1)));
                premiere.setTotal(rs.getString(2));
                premiere.setTicketId(rs.getString(3));
                premiere.setPayment(rs.getString(4));
                premiere.setDateTicket(rs.getString(7));
